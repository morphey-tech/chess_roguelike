...
namespace Project.Unity.Unity.Prepare
{
    ...
    public sealed class PrepareViewFactory
    {
        ...
        public GameObject CreateSlot(GameObject prefab, Vector3 position, Transform parent)
        {
            ...
            return _assetService.InstantiateFromPrefab(prefab, position, Quaternion.identity, parent);
        }

        public GameObject CreateFigure(GameObject controllerPrefab, GameObject viewPrefab, Vector3 position, Transform parent)
        {
            if (controllerPrefab == null || viewPrefab == null) return null;
            var root = _assetService.InstantiateFromPrefab(controllerPrefab, position, Quaternion.identity, parent);
            var view = _assetService.InstantiateFromPrefab(viewPrefab, Vector3.zero, Quaternion.identity, root.transform);
